...
type ChatAuthenticatedShellProps = {
  session: Session;
};

export function ChatAuthenticatedShell({ session: initialSession }: ChatAuthenticatedShellProps) {
  const {
    isOnline,
    isLowBandwidthMode,
    isLowSignal,
  ...
  /** Inbound decrypt produced key mismatch â€” recipient may have rotated keys. */
  ...
  /** Profile fetch failed (nonâ€“no-row) during E2EE bootstrap; do not fall through to keygen. */
  ...
  /** Push / VAPID registration only after startup â€” does not compete with message fetch. */
  useEffect(() => {
  ...
  // Realtime: partner accepted/declined or row inserted â†’ refresh thread gatekeeping.
  useEffect(() => {
  ...
  // Realtime: peer wiped the thread â€” clear local messages + offline snapshot for this pair.
  useEffect(() => {
    if (!session?.user?.id || !activeRecipientId) return;
    const me = session.user.id;
    const other = activeRecipientId;
      ...
      try {
        ...
        if (stored) {
          ...
          if (parsed.publicKeyBase64 && parsed.privateKeyBase64) {
            ...
            // Kill switch: server has a registered key and local disagrees â€” discard local
            ...
            if (pubRaw && localPubTrim !== pubRaw) {
              ...
              // No mismatch: either no server key yet (new user) or local matches server â€” safe to import local pair.
  ...
  useEffect(() => {
    ...
    const ensurePublicKeyUploaded = async () => {
      ...
      try {
        ...
        if (
          error &&
          // PGRST116 = no rows found â€” UPDATE below affects 0 rows until a profiles row exists.
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (error as any).code !== "PGRST116"
        ) {
  ...
  return (
    ...
    >
      <main className="flex min-h-0 w-full min-w-0 flex-1 flex-col">
          ...
          <div className="mx-auto w-full max-w-full space-y-4 lg:max-w-2xl">
            {activeRecipientId ? (
              <div
                ...
                role="status"
              >
                ðŸ”’ Privacy Mode: Messages are automatically deleted every 30 days.
              </div>
            ) : null}
        ...
        {/* Input â€” hidden for recipient until they accept the message request */}
        <div
