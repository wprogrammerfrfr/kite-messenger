...
function statusStyle(status: BridgeStatus) {
  if (status === "connected") {
    return { text: "Connected", color: "text-emerald-400", border: "border-emerald-500/40" };
  }
  if (status === "failed") {
...
export default function StudioTestPage() {
  ...
  useEffect(() => {
    ...
    const appendIceCandidate = async (sessionId: string, nextCandidate: IceCandidateRow) => {
    ...
    const applyRemoteIce = (incoming: IceCandidateRow[] | null | undefined, myRole: Role) => {
    ...
    const init = async () => {
      try {
        ...
        // CRITICAL: keep exact casing for URL session_id -> DB lookup.
        ...
        if (isHost) {
          ...
          // Apply after peer init (so simple-peer has a signal target).
          existingRow = fetched;
        }
        ...
        const enqueueIceAppend = (nextCandidate: IceCandidateRow) => {
          ...
          // Fire-and-forget (do not block simple-peer 'signal' event).
          ...
          try {
            ...
            if (candidateLike) {
              const candidateRecord: IceCandidateRow = {
                ...
                from: activeRole,
                candidate: signalData, // jsonb object (not a string)
              };
        ...
        // Subscribe once peer exists (and before applying initial signals, to avoid missing updates).
        ...
        if (!isHost) {
          if (existingRow?.offer && typeof existingRow.offer === "object") {
            appliedRemoteSignalRef.current = true;
            addLog("Initial offer applied");
            peer.signal(existingRow.offer);
            setStatusNote("Creating answer...");
          }
          if (existingRow?.ice_candidates) {
            addLog("Initial ICE loaded");
            applyRemoteIce(existingRow.ice_candidates, activeRole);
          }
        } else {
          // Host: if peer wrote answer/candidates very quickly, catch it.
    ...
    <div className="min-h-screen bg-stone-950 text-white p-8 flex flex-col items-center justify-center">
      <div className="bg-stone-900 p-8 rounded-2xl border border-stone-800 shadow-2xl max-w-md w-full text-center">
        ...
        <div className={`py-4 px-6 bg-stone-950 rounded-xl border mb-4 ${badge.border}`}>
          <span className="text-sm text-stone-500 uppercase tracking-widest font-semibold">Status</span>
          ...
          {roomId ? <p className="mt-2 text-[11px] text-stone-500">Room: {roomId}</p> : null}
        </div>
